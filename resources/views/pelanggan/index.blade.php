...
@section('content')
    ...
    <a href="<?= url('') ?>/admin/pelanggan/create" class="btn btn-success"><i class="fa fa-plus"></i> Tambah</a>
    <form action="<?= url('') ?>/admin/pelanggan/exportpdf" class="d-inline" method="POST">
        @csrf
        <input type="hidden" name="q" value="{{request('q')}}">
        <button type="submit" class="btn btn-warning"><i class="fa fa-print"></i> Export PDF</button>
    </form>
    <form action="<?= url('') ?>/admin/pelanggan/exportexcel" class="d-inline" method="POST">
        @csrf
        <input type="hidden" name="q" value="{{request('q')}}">
        ...
        <div class="datatable-content p-4">
            <div class="datatable-search-wrap d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                    ...
                    <form action="<?= url('') ?>/admin/pelanggan" class="mx-2">
